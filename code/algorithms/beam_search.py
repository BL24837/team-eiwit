...
#beam_width = 200 # Het maximaal aantal configuraties dat per stap wordt bijgehouden (breedte van de beam).
...
# Simulatieparameters
sequence = "HCPHPHPHCHHHHPCCPPHPPPHPPPPCPPPHPPPHPHHHHCHPHPHPHH"
beam_widths = range(1, 50)
scores = []

# Voer de simulatie uit voor elke beam width
for beam_width in beam_widths:
    best_protein = beam_search_protein_folding_with_class(sequence, beam_width)
    score = best_protein.calculate_stability()
    scores.append(score)

# Grafiek maken
plt.figure(figsize=(10, 6))
plt.plot(beam_widths, scores, label="Stability Score")
